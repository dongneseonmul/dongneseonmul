# Cloudflare D1 수동 마이그레이션 가이드

API 토큰 권한 문제로 CLI를 통한 마이그레이션이 불가능하므로, Cloudflare Dashboard에서 직접 SQL을 실행해야 합니다.
...
### 2. 스키마 생성 (아래 SQL 전체 복사하여 실행)

아래 SQL을 복사하여 Console에 붙여넣고 **Execute** 버튼 클릭:
...
스키마 생성 후, 아래 SQL로 샘플 데이터를 삽입할 수 있습니다:
...
- [ ] Cloudflare Dashboard에서 webapp-production 데이터베이스 접속
- [ ] Console에서 스키마 생성 SQL 실행
- [ ] 샘플 데이터 삽입 SQL 실행 (선택)
- [ ] 테이블 생성 확인 (`SELECT name FROM sqlite_master WHERE type='table';`)
- [ ] API 엔드포인트 테스트 (`curl https://dongneseonmul.pages.dev/api/gifts`)

## 🔗 참고 링크

- **Cloudflare Dashboard**: https://dash.cloudflare.com
- **D1 Database Console**: https://dash.cloudflare.com → Workers & Pages → D1 → webapp-production → Console
- **프로덕션 URL**: https://dongneseonmul.pages.dev
